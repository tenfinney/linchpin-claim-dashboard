 ...
 */
export const displayIdentity = (publicKey) => {
    const shortId = publicKey.substr(publicKey.length - 11, 10);
    console.log('displayIdentity', shortId);
    // return (<strong>identity:{shortId}</strong>)
 ...
 */
export const displayAvatar = (identity) => {
 ...
 */
export const displayThreadsList = (threads) => {
    console.log('displayThreadsList', threads);
    /** Timeout just so the UI changes some after it loads :) */
    // setTimeout(() => {
